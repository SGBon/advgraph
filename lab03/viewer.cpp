...
void init() {
  ...
  int result = stat("buddha.bin",&buf);
  if(!result){
    int fid = open("buddha.bin", O_RDONLY | O_BINARY);
    ...
    int fid = open("buddha.bin", O_WRONLY | O_BINARY | O_CREAT, S_IREAD | S_IWRITE);
